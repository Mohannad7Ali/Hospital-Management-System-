...
class CreateGroupServices extends Component
{
    ...
    {
        $Total_after_discount = $this->subtotal;
    ...
    public function subtotal()
    {
        ...
        foreach ($this->GroupsItems as $groupItem) {
            if ($groupItem['is_saved'] && $groupItem['service_price'] && $groupItem['quantity']) {
                $totalBeforeDiscount += $groupItem['service_price'] * $groupItem['quantity'];
            }
        }
        $subtotal = 0;
        $subtotal = $totalBeforeDiscount - ((is_numeric($this->discount_value) ? $this->discount_value : 0));
        return $subtotal;
    }
    ...
    // click add services button => make new empty element group items
    public function addService()
    {
        // you have to save all your services before adding new
    ...
    // save service that mean fill the empty object in group item that we make by click add service
    ...
    public function delete($id)
    {
        ...
        Group::destroy($id);
        return redirect()->to('/Add_Services_Group');
    }
}
